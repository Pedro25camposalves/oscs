...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    header{padding:20px 24px; display:flex; align-items:center; gap:16px; background:linear-gradient(90deg, rgba(255,255,255,0.9), rgba(255,255,255,0.6)); box-shadow:0 1px 4px rgba(0,0,0,0.06)}
    header h1{font-size:18px; margin:0}
    main{padding:20px; max-width:1100px; margin:20px auto}
    form{display:grid; gap:18px}

    .card{background:var(--card-bg); border-radius:10px; padding:16px; box-shadow:0 6px 18px rgba(16,24,40,0.04)}
    .card h2{margin:0 0 12px 0; font-size:16px}
    .grid{display:grid; gap:12px}
    .cols-2{grid-template-columns:1fr 1fr}
    .cols-3{grid-template-columns:repeat(3,1fr)}
    label{display:block; font-size:13px; color:var(--muted); margin-bottom:6px}
    input[type="text"], input[type="color"], input[type="file"], textarea, select{width:100%; padding:8px 10px; border-radius:8px; border:1px solid #e6e6e9; font-size:14px}
    textarea{min-height:80px; resize:vertical}
    .row{display:flex; gap:12px; align-items:center}
    .small{font-size:12px; color:var(--muted)}

    .images-preview{display:flex; gap:10px; flex-wrap:wrap; margin-top:8px}
    .images-preview img{width:120px; height:70px; object-fit:cover; border-radius:6px; border:1px solid #eee}

    .divider{height:1px; background:#efefef; margin:8px 0}
    .section-title{font-weight:600; color:var(--text); margin:6px 0}

    .directors-list{display:flex; gap:10px; flex-wrap:wrap; margin-top:12px}
    .director-card{background:#fafafa; padding:8px; border-radius:8px; display:flex; gap:8px; align-items:center; border:1px solid #f0f0f0}
    .director-card img{width:48px; height:48px; border-radius:6px; object-fit:cover}
    ...
    .modal{background:white; width:520px; max-width:94%; border-radius:10px; padding:16px}
    ...
    pre.json-out{white-space:pre-wrap; background:#111; color:#e6e6e6; padding:12px; border-radius:8px; font-size:12px}
  </style>
</head>
<body>
  <header>
    ...
    <form id="oscForm" class="card" onsubmit="event.preventDefault();saveData()">
            ...
            <div id="previewArea">
              ...
              <div style="margin-top:12px">
                ...
                <div class="row" id="colorSwatches">
                  <div style="padding:8px; border-radius:8px; min-width:80px; text-align:center">BG<br><div id="swBg">&nbsp;</div></div>
                  <div style="padding:8px; border-radius:8px; min-width:80px; text-align:center">Sec<br><div id="swSec">&nbsp;</div></div>
                  <div style="padding:8px; border-radius:8px; min-width:80px; text-align:center">Ter<br><div id="swTer">&nbsp;</div></div>
                  <div style="padding:8px; border-radius:8px; min-width:80px; text-align:center">Qua<br><div id="swQua">&nbsp;</div></div>
                </div>
              </div>
      ...
      <div class="card">
        ...
        <div class="grid cols-3">
          ...
          <div>
            <label for="projetos">Projetos ativos</label>
            <input id="projetos" type="text" />
          </div>
          <div>
            <label for="beneficiados">Beneficiados</label>
            <input id="beneficiados" type="text" />
          </div>
  ...
  <script>
    ...
    async function updatePreviews(){
      ...
      if(l1){ const src = await readFileAsDataURL(l1); const img=document.createElement('img'); img.src=src; previewLogoSimples.appendChild(img) }
      if(l2){ const src = await readFileAsDataURL(l2); const img=document.createElement('img'); img.src=src; previewLogoCompleta.appendChild(img) }
      [b1,b2,b3].forEach(async (b)=>{ if(b){ const src = await readFileAsDataURL(b); const img=document.createElement('img'); img.src=src; previewBanners.appendChild(img) } })
    ...
    openDirectorModal.addEventListener('click', ()=>{ modalBackdrop.style.display='flex' });
    closeDirectorModal.addEventListener('click', ()=>{ modalBackdrop.style.display='none' });
    ...
    async function addDirector(){
      ...
      if(!nome || !func){ alert('Preencha nome e função do diretor'); return }
      const fotoData = foto ? await readFileAsDataURL(foto) : null;
      const dir = {foto: fotoData, nome, func};
      directors.push(dir);
      renderDirectors();
      // reset modal fields
      qs('#dirFoto').value=''; qs('#dirNome').value=''; qs('#dirFunc').value='';
      modalBackdrop.style.display='none';
    }
    addDirectorBtn.addEventListener('click', addDirector);

    function renderDirectors(){
      const list = qs('#directorsList'); list.innerHTML='';
      directors.forEach((d, i)=>{
        const c = document.createElement('div'); c.className='director-card';
        const img = document.createElement('img'); img.src = d.foto || 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48"><rect width="100%" height="100%" fill="%23eee"/></svg>';
        const info = document.createElement('div'); info.innerHTML = `<div style="font-weight:600">${escapeHtml(d.nome)}</div><div class="small">${escapeHtml(d.func)}</div>`;
        const remove = document.createElement('button'); remove.className='btn'; remove.textContent='✕'; remove.style.padding='6px 8px'; remove.style.marginLeft='8px'; remove.addEventListener('click', ()=>{ directors.splice(i,1); renderDirectors() });
        c.appendChild(img); c.appendChild(info); c.appendChild(remove);
        list.appendChild(c);
      })
    }
  ...
  async function saveData() {
    ...
    const data = {};
    data.labelBanner = qs("#labelBanner").value;
    ...
    data.logoSimples = await uploadImage(logoSimples.files[0]);
    data.logoCompleta = await uploadImage(logoCompleta.files[0]);
    data.banner1 = banner1.files[0] ? await uploadImage(banner1.files[0]) : null;
    data.banner2 = banner2.files[0] ? await uploadImage(banner2.files[0]) : null;
    data.banner3 = banner3.files[0] ? await uploadImage(banner3.files[0]) : null;
    // ------------------------------------------
    ...
    data.transparencia = {
      recursos: qs("#recursos").value,
      projetos: qs("#projetos").value,
      beneficiados: qs("#beneficiados").value,
      nomeFantasia: qs("#nomeFantasia").value,
      sigla: qs("#sigla").value,
      situacaoCadastral: qs("#situacaoCadastral").value,
      endereco: qs("#endereco").value,
      ...
      abreviacao: qs("#abreviacao").value,
    };
    ...
    function resetForm(){ if(confirm('Limpar todos os campos?')){ document.getElementById('oscForm').reset(); directors.length=0; renderDirectors(); updatePreviews(); qs('#jsonOut').textContent='{}'; qs('#downloadLink').style.display='none'; } }
